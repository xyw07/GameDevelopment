...
class Pacman(GameCharacter):
    ...
    def update(self):
        ...
        # use self.on_left instead of self.x directy,
        # using a range of position instead of a specific position
        if self.on_left:
            self.maze.eat_dots(self.maze.LEFT_VERT, self.y)
        if self.on_right:
            self.maze.eat_dots(self.maze.RIGHT_VERT, self.y)
        if self.on_top:
            self.maze.eat_dots(self.x, self.maze.TOP_HORIZ)
        if self.on_bottom:
            self.maze.eat_dots(self.x, self.maze.BOTTOM_HORIZ)
